...
*   Includes personal changes to how the 'full size' is calculated, it is now based on the total size of a directory, which includes individual files and files within subfolders.
*   Includes a custom storage capacity setting to allow user to define their total storage capacity instead of relying system defaults (useful for shared hosting).
*   Replace browser video player with Plyr and add seeking support.
